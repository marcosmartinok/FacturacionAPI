...
public class FacturasDbContext : DbContext
{
	...
	protected override void OnModelCreating(ModelBuilder builder)
	{
		...
		builder.Entity<Factura>(entity =>
		{
			...
			entity.Property(x => x.IVATotal).IsRequired().HasColumnType("DECIMAL(3, 2)");
			entity.HasOne<Empresa>(x => x.Empresa).WithMany(x => x.Facturas).HasForeignKey(x => x.IdEmpresa).OnDelete(DeleteBehavior.SetNull);
			entity.HasOne<Persona>(x => x.Persona).WithMany(x => x.Facturas).HasForeignKey(x => x.IdPersona).OnDelete(DeleteBehavior.SetNull);
		});
		...
		builder.Entity<LineaFactura>(entity =>
		{
			...
			entity.Property(x => x.IVA).IsRequired();
